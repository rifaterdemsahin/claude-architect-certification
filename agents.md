...
### 2026-07-18
- **Task:** 🧠 Add a Self-Learning menu under Production + 🎨 add a Canva animation-building Google Drive folder to Post-Production tools.
- **Action:** Found `5_Symbols/production/preprod/self_learning.html` already existed but was orphaned (no nav link anywhere) and its `shared/nav.js`/`debug-panel.js` script tags had an extra `../` (4 instead of 3), so the page rendered with **no navigation at all**. Fixed the path bug, added the page as a step card in `5_Symbols/production/preprod/index.html`, and added it to the debug-menu fallback arrays in `navigation_config.json`, `index.html`, `markdown_renderer.html`. Discovered the **live Project Menu is actually Supabase-backed** (`navigation_menus` table, `shared/nav.js`) — `navigation_config.json` is only a fallback. Preprod already had a live "Self Learning" group (3 items); **Production did not** — that was the real gap. Used the `5_Symbols/production/preprod/tools/menu_builder.html` admin tool to add a "5. 🧠 Self Learning" group under Production (sort order 5, matching sibling groups). Separately added a "Canva Animation Building" card (Drive folder `1wlUpWAVZGbMK_W3DDR2uXa4xGBualgfC`) to `5_Symbols/production/postprod/index.html`'s 🛠 Tools group and to the Post Prod → Tools → Design nav entries in all three nav files. A stray `edit-decision-list.fly.dev` URL in the original prompt referenced an unrelated sibling project (`~/projects/edit-decision-list`) — left untouched.
- **Verification:** Freed port 8080 (was occupied by the sibling `edit-decision-list` server), rebuilt/ran this repo's own Go server. Confirmed in Chrome: `self_learning.html` now renders the shared nav and is reachable from the Preprod menu; `postprod/index.html` shows the new Canva card. Documented in `4_Formula/llm_thinking_log.md` and `prompts.md`.
- **Status:** Static-file changes IMPLEMENTED, COMMITTED; Supabase `navigation_menus` "Self Learning" group under Production save is IN PROGRESS (menu_builder.html tab hung after clicking Save — resuming).
...
- **🌐 Remind to Verify on fly.dev After Push:** After committing and pushing a fix that affects the production deployment (e.g., shared JS, server handlers, templates), always remind the user to open `https://claude-architect-certification.fly.dev/` (and any specific affected page) in Chrome so they can verify the fix on the live production site. Example: `open -a "Google Chrome" https://claude-architect-certification.fly.dev/5_Symbols/production/preprod/research/images.html`.
