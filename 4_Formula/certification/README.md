# 📚 certification — Course & Certification Planning

> **Purpose:** Course outline, exam design, and production planning for the Claude Architect Certification.

## Files

| File | Description |
|------|-------------|
| `course_outline.md` | Module-by-module course structure |
| `exam_and_case_study.md` | Exam questions and case study design |
| `post_prod_template.md` | Post-production review template |
| `production_plan.md` | Video and content production schedule |

## Rules
- Link each module back to the corresponding `5_Symbols/production/` assets
- Keep exam questions validated against `7_Testing_Known` outcomes