...
3. **Manual Generation**: Changed the trigger so the user clicks "Generate Slides ⚡" explicitly to build the presentation, rather than generating automatically on every checkbox tick.
4. **Download & Copy**: The UI builds the Marp markdown string dynamically in an editable `<textarea>`, providing a "📋 Copy" button alongside the "Download .md".
5. **Side-by-Side Preview**: Recreated `marp.live` dual-pane layout. Left pane holds the markdown; right pane renders a visual mock of the current slide using HTML/CSS grid.
6. **Supabase Tables (Proposed)**: 
