...
- **OpenRouter I/O inspector modal** → overlay (`#ioInspectorOverlay`) showing the exact prompt sent + model output with copy-prompt/output/all. Includes a **real-time search input** (`#ioSearch`) that highlights matching terms in both the prompt and output (e.g., to verify the negative prompt). Closes on ✕, backdrop click, or the **Escape** key (global `keydown` listener).
