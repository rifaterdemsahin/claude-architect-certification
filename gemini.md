...
- **⚡ Mandatory Server Startup & Display on Login/Restart** — Whenever starting a session or after a system/session restart, ALWAYS turn on the local application server on localhost:8080 immediately (`go build ./cmd/server && ./server` run asynchronously in background), open `http://localhost:8080/` in Google Chrome (`open -a "Google Chrome" http://localhost:8080/`), AND display the clickable link http://localhost:8080/ at the very start of your initial response. DO NOT make the user ask or prompt for the server to be started or shown. Verify if it is running first (`curl -s http://localhost:8080 >/dev/null`) and start it automatically if not running.
- **🚀 Mandatory Post-Completion Workflow** — Upon task completion, always perform the following sequence: 1. Run `go build` to verify system integrity; 2. Start the local server (if not running); 3. Open the specific page(s) that were updated in **Google Chrome** (`open -a "Google Chrome" <url>`) for final visual and functional verification.
