...
- **⚡ Mandatory Server Startup on Login/Restart** — Whenever starting a session or after a system/session restart, ALWAYS turn on the local application server on localhost:8080 immediately (`go build ./cmd/server && ./server` run asynchronously in background). DO NOT make the user ask or prompt for the server to be started. Verify if it is running first (`curl -s http://localhost:8080 >/dev/null`) and start it automatically if not running.
